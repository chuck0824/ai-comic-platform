...
interface CounterProps {
  ...
  suffix?: string; prefix?: string; duration?: number; decimals?: number
}
...
function Counter(props: CounterProps) {
  ...
  const formatValue = useCallback(
    (v: number) => decimals > 0 ? v.toFixed(decimals) : Math.round(v).toLocaleString(),
    [decimals]
  )
  ...
  useEffect(() => {
    ...
    if (mq.matches) { el.textContent = `${prefix}${formatValue(end)}${suffix}`; return }
    const observer = new IntersectionObserver(([entry]) => {
      if (entry.isIntersecting && !startedRef.current) { startedRef.current = true; animate(); observer.unobserve(el) }
    }, { threshold: 0.5 })
    observer.observe(el)
        ...
        <div className='grid grid-cols-2 gap-8 md:grid-cols-4 md:gap-12'>
          {stats.map((s) => (
            <div key={s.label} className='flex flex-col items-center text-center'>
              <span className='bg-gradient-to-r from-pink-400 via-indigo-400 to-blue-500 bg-clip-text text-transparent text-2xl font-bold tracking-tight md:text-3xl'>
