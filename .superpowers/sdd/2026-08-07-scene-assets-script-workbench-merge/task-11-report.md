...
针对审查提出的五项事实偏差，先新增四组契约断言；第一次执行为 `6 / 10` 通过、`4 / 10` 失败，分别锁定：当前实现与目标计费合同未分离、API 状态枚举错误、场景变体/快照/归档合同不符、组件/V16/错误码来源不准确。完成六文档修订后，同一命令为 `10 / 10` 通过。最终覆盖：
...
| 3 | 基础错误 | 组件名、`scene_variant_version_id`、V16 内容及“项目归档”写错 | 改为 `SceneAssetDetailDrawer.vue`、`scene_variant_version`、V16 五列/两个索引、仅锁分镜 | 高 |
...
| 7 | 描述准确性 | 快照被描述为独立 fingerprint/锁定列，变体操作被描述为不增母资产版本 | 按 DTO/Service 改为 `scene_asset_snapshot` JSON、storyboard version 锁定；变体写操作 append 资产版本 | 中 |
| 8 | 敏感信息 | 模型来源描述可能诱导浏览器直连管理端或把本地记录当真实流水 | 保留同源 BFF、无密钥规则；明确权威账务只能来自 3001 | 中 |
| 9 | 确认落实 | 审查五项未被自动测试约束 | 新增四组防回归测试，覆盖状态、计费边界、变体/快照/归档、组件/V16/错误来源 | 低 |
...
- `node --test tests/script-workbench-docs.test.cjs`：`10 / 10` 通过。
