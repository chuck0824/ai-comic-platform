...
点击确认按钮后不得直接跳转，必须进入独立的**阶段过渡进度页**。页面展示：

- 当前阶段与下一阶段、阶段确认状态、任务编号和请求编号。
- 三段进度：排队校验、准备上下文、开放下一阶段。
- 任务状态 `QUEUED`、`RUNNING`、`SUCCEEDED`、`FAILED`、`CANCELED`。
- `QUEUED/RUNNING` 时允许取消并返回当前阶段；取消不丢失当前产物。
- `FAILED` 时保留错误原因、当前版本和“重新尝试”按钮；重试创建新任务并关联原任务。
- `SUCCEEDED` 时才将下一阶段置为 `READY`，并显示“立即进入下一阶段”。
...
验收要求：连续双击确认只创建一个活动任务；刷新过渡页可恢复真实状态；取消后仍停留在原阶段；失败可重试；任务成功前下一阶段不可进入；成功后按钮与左侧阶段导航同时开放。
...
检查结果分为错误、警告、建议。错误必须修复才能进入步骤 6；警告允许用户确认后继续。
...
| M3 剧本与分镜 | 剧本正文、检查、文字分镜、三类导出 | 可输出至少一集有效文字分镜 |
| M4 稳定性 | 权限、审计、历史对比、可观测性、性能优化 | 达到上线非功能门槛 |

### 16.2 优先级

- P0：六步主链路、异步任务、保存、版本、上下游过期、基础权限、错误恢复、按集/场景局部重新生成、基础一致性检查、分镜导出。
...
4. 选择高压开场并生成前 3 集结构。
5. 生成第 1 集剧本，修复一致性错误。
6. 生成第 1 集文字分镜，完成检查并导出 JSON/XLSX/Markdown。
...
| A10 | 生成模型和提示词模板不向普通用户开放 | 管理员策略配置中心 |
