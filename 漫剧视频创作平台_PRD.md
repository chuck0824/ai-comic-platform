...
| 生成成功 | 实际积分、返还积分、任务状态、结果决策 | 生成成功即结算，用实际 quota 结算并返还差额；此时决策为 `PENDING` |
...
结算与内容决策是两条独立状态轴：任务 `SUCCEEDED` 时只写入一次积分流水与结果记录；用户随后“采纳”只把结果决策更新为 `ACCEPTED` 并创建产物新版本，“放弃”只更新为 `DISCARDED`，不删除成功任务、不撤销已经发生的消费，也不写入产物。重复采纳、放弃或重试不得产生第二笔结算。
...
3. 生成成功即结算并保存 `PENDING` 结果。用户采纳结果时创建新版本、保留历史并精确标记下游 `STALE`；放弃结果不覆盖产物，但成功任务、完整计价快照和积分流水仍可回访。同一 `taskId` 重复采纳或放弃必须幂等。
