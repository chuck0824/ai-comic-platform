...
| 小说分析 | 小说上传、解析成功且原文已确认 | “下一步”置灰，展示解析状态 |
| 改编方案 | 小说分析存在成功版本 | 展示缺失项和返回步骤 2 入口 |
| 分集结构 | 改编方案已确认 | 展示“请先确认改编方案” |
| 剧本正文 | 至少一集分集结构已确认 | 仅开放已有结构的集数 |
...
stateDiagram-v2
    ...
    RUNNING --> SUCCEEDED: 生成并持久化成功
    RUNNING --> FAILED: 模型/解析/服务异常
    RUNNING --> CANCELED: 取消生效
...
- 演示模型只返回确定性的示例结果，必须标注“演示”，固定消耗 0 积分，不可伪装成真实模型输出。
...
按次定价或分层表达式模型由 3001 直接返回预计 quota；平台只做单位名称映射。缓存、音频、图片等扩展倍率同样以 3001 最终账单为准。
...
| 生成成功 | 实际积分、返还积分、输出版本 | 用实际 quota 结算，返还差额 |
...
- 标准条件码至少包括：`SCRIPT_BLOCK_REQUIRED`（需选中正文块）、`REVIEW_BLOCKED`（需解决 `HIGH`/`BLOCKER` 问题）、`SHOT_REQUIRED`（需选中镜头）、`STAGE_ARTIFACT_REQUIRED`（需先保存当前阶段产物）、`STORYBOARD_HISTORY_REQUIRED`（暂无可撤销版本）、`PROJECT_ARCHIVED`（归档项目只读）。
- 引导后的目标元素需滚动到可见区并显示与现有界面一致的短暂聚焦态；审核高风险问题、分镜表格行与卡片都必须可见。
- Toast 只是补充反馈，不能仅以 Toast 作为业务动作的唯一结果。

### 17.2 生成与结果状态

1. 通过守卫后，AI 动作创建具有稳定 `taskId` 的任务，并进入 `QUEUED → RUNNING → SUCCEEDED | FAILED | CANCELED`。进度只能在 `SUCCEEDED` 时达到 100%，失败保留原产物并允许重试。
2. `generation-progress` 展示范围、模型、预计积分和进度；`generation-result` 展示生成前后差异、任务、版本、Markdown 路径和积分。
3. 用户采纳结果时创建新版本、保留历史并精确标记下游 `STALE`；放弃结果不覆盖产物，但任务记录仍可回访。同一 `taskId` 重复采纳必须幂等。
4. 人工保存、检查、审核、导出、归档和交接也必须生成可回访的 `SUCCEEDED` 结果记录，演示任务记录实际积分 0。
5. 任务中心保留所有生成与人工结果。已采纳的生成结果回访时只读，不再写入版本或重复计费。
