...
| 文档类型 | PRD |
| 目标版本 | V1.0 |
| 核心目标 | 搭建可用的自由画布创作闭环，完成剧本到分镜图、视频片段、基础合成的最小生产链路 |

## 2. 一期定位

一期聚焦“能创作、能组织、能生成、能复用”的核心闭环。

用户需要在一个自由画布中完成剧本输入、基础节点组织、脚本拆解、角色/场景/道具资产管理、分镜图批量生成、视频片段生成、基础视频合成。

一期不追求高级图像编辑、复杂视频剪辑、3D 导演台和多人协同，优先保证主链路跑通。
...
一期视频节点只实现基础生成和预览，不实现高级剪辑、解析、高清、人声分离。
...
| 视频到合成 | 视频节点 → 视频合成节点 | 视频片段进入合成时间轴 |
| 音频到合成 | 音频节点 → 视频合成节点 | 音频作为配音或背景音乐 |
...
| 功能 | 说明 |
| --- | --- |
| 创建合成节点 | 多个视频节点连接后创建视频合成节点 |
| 顺序拼接 | 按连接顺序或时间轴顺序拼接视频 |
| 添加音频 | 支持添加一个 BGM 或配音音频 |
| 预览播放 | 支持合成前预览 |
| 生成成片 | 点击合成生成最终视频 |
| 下载视频 | 合成完成后下载 |
...
| 文本模型 | 单选框 | 调用“模型配置-语言模型”中已启用模型 | 该选项为单一选项 |
| 生成需求 | 多行文本框 | 用户输入 | 用户描述希望生成的文本 |
| 生成文本 | 按钮 | 无 | 点击后校验模型和生成需求，创建文本生成任务 |
| 应用结果 | 按钮 | 文本生成结果 | 将生成结果写入文本内容 |
...
| 字段/按钮 | 类型 | 数据来源 | 交互说明 |
| --- | --- | --- | --- |
| 视频片段列表 | 可排序列表 | 上游视频节点 | 支持拖拽调整顺序 |
| 音频轨 | 单选引用 | 上游音频节点 | 一期支持一个音频轨 |
| 是否保留原声 | 开关 | 用户选择 | 默认开启 |
| 预览 | 按钮 | 当前片段列表 | 生成预览或播放已有预览 |
| 生成成片 | 按钮 | 无 | 提交合成任务 |
| 下载 | 按钮 | 合成结果 | 合成成功后展示 |
