...
> 日期：2026-06-16  
> 状态：评审中  
> 🆕 V1.5修订：产品不再定位为单一 AI 视频生成工具或简单自由画布，而是面向 AI 漫剧、短剧、广告片、短视频创作者的 **AI 漫剧与视频内容工业化生产工作台**。核心能力以无限画布、节点系统、资产库、分镜生产、批量生成、视频合成、工作流模板、Agent / Skill 自动执行共同构成生产闭环。
...
**AI漫剧与视频内容工业化生产工作台** — 通过无限画布、节点系统、资产库、分镜生产、批量生成、视频合成、工作流模板与 Agent / Skill 自动执行，完成从剧本到成片的工业化生产闭环。
...
| **解决什么** | AI工具分散、单次生成不可控、角色/场景资产无法沉淀、分镜到成片链路断裂、团队生产不可追溯 |
| **差异化** | 以「项目→画布→节点→连线→分镜→资产→批量生成→时间线→导出→工作流→Agent/Skill」组织完整生产系统，竞争力不只在单次生成效果，而在工程化、标准化、可追溯、可复用和可规模化 |
...
| 画布 | 镜头生产、节点编排、资产生成、时间线、导出 | 不反向覆盖剧本Master，除非用户选择强耦合同步 |
...
| 视频合成时间线 | 多视频、多音轨、字幕、BGM、音效、导出任务 | P0 |
...
|---|---|---|---|---|
| 页面布局 | 用户在一个无限画布工作台完成节点编排、生成、资产沉淀和合成 | 左侧栏、中央画布、右侧属性面板、底部生成器、小地图、顶部项目栏必须同时可用 | 提供项目、节点、连线、任务、资产、时间线聚合接口 | 校验各区域在空项目、已有项目、加载失败、权限不足下都能正常展示 |
...
| 字段 | 必填说明 |
|---|---|
| 节点名称/类型 | 例如脚本节点、图片节点、视频节点、音频节点、角色节点、合成节点 |
...
| 结果位置 | 输出回写到节点、分镜行、资产库、时间线或导出中心的具体位置 |
...
| 视频工具 | 高清放大、帧率提升、视频解析、视频剪辑、人声/背景声分离、视频合成半屏时间线 | P0/P1 | 解析结果可全屏表格查看；剪辑支持快捷键；合成支持预览和导出 |
...
```
跑通「项目创建 → 剧本导入/创意输入 → AI拆分镜 → 资产沉淀 → 批量生图/生视频 → 时间线合成 → 成片导出 → 工作流复用」最小生产闭环
```

具体目标：
1. **创作闭环**：用户可在30分钟内从一句话创意到导出1集成片
...
| F-41 | 画布 | 时间轴编辑器（视频轨+配音轨+字幕轨） | M |
| F-42 | 画布 | 配音生成（10音色+情绪+语速） | M |
| F-43 | 画布 | 字幕自动打轴+样式选择 | M |
| F-44 | 画布 | BGM添加+音效标记 | S |
| F-45 | 画布 | 图文漫剧合成（静态图+Ken Burns运镜） | M |
| F-46 | 画布 | 画布实时预览（低分辨率逐镜头） | M |
| F-47 | 画布 | MP4导出（9:16/16:9，720p/1080p） | M |
...
| 🆕 F-123 | 画布 | 时间轴多轨编辑器（5轨道+半屏剪辑+20min长视频） | M |
...
### 5.5 V1.3 新增功能（32项）— AI视频创作自由画布增强版
...
| 🆕 F-133 | 脚本 | 脚本节点全流程流水线：剧本输入→分镜解析→批量生图→批量生视频→配音字幕→合成 | M |
...
| 🆕 F-137 | 视频 | 视频节点下游菜单：配音/字幕/剪辑/拼接/转场/特效叠加 | M |
| 🆕 F-138 | 参考 | 全能参考视频引擎：上传参考视频→AI解析构图/光影/色调/运镜/节奏→应用到生成 | M |
| 🆕 F-139 | 参考 | 多模态参考融合：参考图+参考视频+文字描述 三通道融合指导生成 | S |
| 🆕 F-140 | 多副本 | 多副本并行抽卡：同Prompt并行生成N张→用户择优→自动选最佳进时间轴 | M |
| 🆕 F-141 | 多副本 | 多副本对比视图：并排展示+差异高亮+一键替换时间轴素材 | M |
| 🆕 F-142 | 资产 | 资产双向联动（画布↔仓库）：画布锁定资产自动同步仓库→仓库资产变更推送到画布 | M |
| 🆕 F-143 | 资产 | 资产版本追溯：画布中资产可查看仓库历史版本→选择特定版本应用 | S |
| 🆕 F-144 | 时间轴 | 时间轴合成增强：7条轨道（视频+配音+字幕+BGM+音效+特效+图层叠加） | M |
| 🆕 F-145 | 时间轴 | 智能转场引擎：AI分析相邻镜头内容→推荐最佳转场类型（硬切/淡入淡出/滑动/模糊/匹配剪辑） | S |
...
| AC03 | 完成后自动跳转剧本资产工作台，展示梗概、大纲、章节正文、分镜初稿和投流素材 |
...
**UI参考**：画布三栏布局 — 左侧分镜卡片面板（可拖拽排序）+ 中间画布编辑区（选中镜头预览+Inpaint工具条）+ 右侧属性面板（景别/运镜/时长/关键帧/角色场景列表）+ 底部时间轴（6条轨道）+ 底部资产面板
...
| **版本** | V1.0 |
| **优先级** | M |
| **用户故事** | 作为创作者，我希望一键合成所有分镜画面+配音+字幕+BGM为完整视频 |
| **前置条件** | 分镜画面已生成、配音已生成、字幕已生成 |
| **关联API** | `POST /canvas/projects/:id/compose` |

**验收标准**：

| AC# | 标准 |
|-----|------|
| AC01 | 点击「合成视频」→ 收集所有素材 → 推入渲染队列 |
| AC02 | 合成进度展示：Phase1画面拼接 → Phase2音频混音 → Phase3字幕叠加 → Phase4编码输出 |
| AC03 | V1.0支持「图文漫剧」模式：静态关键帧 + Ken Burns运镜(推拉摇移) + 配音 + 字幕 + BGM |
| AC04 | 转场默认硬切，用户可在时间轴选择转场效果（淡入淡出/滑动） |
| AC05 | 合成完成后通知 → 可预览 → 不满意可回到画布修改后重新合成 |
| AC06 | 渲染失败 → 展示失败原因 → 支持重试 |
...
| **版本** | V1.0 |
| **优先级** | M |
| **用户故事** | 作为创作者，我希望将合成的视频导出为MP4文件下载到本地 |
| **前置条件** | 视频已合成 |
| **关联API** | `POST /canvas/projects/:id/export` → `GET /canvas/export/:task_id/download` |

**验收标准**：

| AC# | 标准 |
|-----|------|
| AC01 | 导出设置：选择画幅(9:16/16:9/1:1) + 分辨率(720p/1080p) + 格式(MP4) + 水印选项 |
| AC02 | 免费用户：强制带平台水印（右下角半透明Logo），720p导出 |
| AC03 | 付费用户：可选无水印，最高1080p导出 |
| AC04 | 提交导出 → 后台渲染 → 完成后站内通知+浏览器推送 |
| AC05 | 下载页面：展示文件信息（名称/大小/时长/分辨率）+ 下载按钮（OSS签名URL，24h有效） |
| AC06 | 下载历史保留30天 → 过期文件标注「已过期」 |
| AC07 | 多集批量导出（V1.2）：勾选1-5集 → 一键打包下载ZIP |
...
| AC# | 标准 |
|-----|------|
| AC01 | 画布底部Slash工具栏提供快捷命令：`/九宫格` `/四宫格` `/25宫格` `/角色三视图` `/光影矫正` `/导演台` `/视频剪辑` `/拆分` `/拼接2K` `/全景720°` |
...
| **版本** | V1.2 |
| **优先级** | M |
| **用户故事** | 作为创作者，我希望在时间轴上直观编辑多轨道（视频/配音/字幕/BGM/音效），像剪辑软件一样精编成片 |
| **前置条件** | 已有视频片段 |
| **关联API** | `PUT /canvas/projects/:id/timeline` |

**验收标准**：

| AC# | 标准 |
|-----|------|
| AC01 | 5条轨道：🎥视频轨 + 🔊配音轨 + 📝字幕轨 + 🎵BGM轨 + 🔔音效轨 |
| AC02 | 视频轨支持拖拽调整片段顺序、裁剪起止点、选中Delete删除 |
| AC03 | 支持多片段顺序拼接，自动统一素材差异，无缝拼接 |
| AC04 | 支持视频原声+独立音频轨(BGM/配音)混合，一键静音/开音 |
| AC05 | 预览画面与时间轴双向联动，空格键播放/暂停，方向键逐帧移动 |
| AC06 | 半屏剪辑模式：收起时间轴为半屏，在画布上实时添加/删除视频 |
| AC07 | 支持20min长视频合成导出 |
...
| **版本** | V1.2 |
| **优先级** | M |
| **用户故事** | 作为创作者，我希望AI能像专业团队一样协作——决策层拆解任务、执行层生成内容、监督层审核质量，确保成片稳定一致 |
| **前置条件** | 项目已创建，AI模型已配置 |
...
| AC# | 标准 |
|-----|------|
| AC01 | 企业概览卡片：团队成员数/本月剧本生成次数/本月成片导出数/资产总数 |
| AC02 | 财务卡片：本月总支出/采购订单数/待审批数/API调用次数 |
| AC03 | 待处理列表：采购申请审批/成片导出审核/成员邀请确认 |
...
| **版本** | V1.3 |
| **优先级** | M |
| **用户故事** | 作为创作者，我希望ScriptAgent(决策)、ProductionAgent(执行)、QualityAgent(监督)三层Agent深度联动，像专业团队一样分工协作，确保从剧本到成片全流程质量可控 |
...
| V10-07 | 画布：导入剧本→自动解析分镜卡片→生成角色/场景图→配音→字幕→合成→导出MP4 | E2E测试 |
...
| 视频合成队列积压 | 导出超时 | 中 | 弹性Worker+优先级队列+超时告警 |
...
| US-034 | 画布 | 时间轴编辑+生成配音 | B | V1.0 | `PUT timeline` + `POST dub` |
| US-035 | 画布 | 合成图文漫剧视频 | ALL | V1.0 | `POST compose` |
| US-036 | 画布 | 导出MP4下载 | ALL | V1.0 | `POST export` |
| US-037 | 画布 | 图生视频模式 | B | V1.2 | 合成模式扩展 |
...
| 🆕 US-051 | 画布 | 时间轴5轨编辑器+半屏剪辑 | ALL | V1.2 | `PUT /canvas/.../timeline/full` |
