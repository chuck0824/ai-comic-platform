...
|---|---|---|
| 新增：Agent 配置中心 | M16 模块：4 系统蓝图、结构化+高级双编辑、版本生命周期、项目绑定、试跑 A/B 对比 | `agent-config-center-design.md` |
| 新增：统一任务中心 | M13 模块：用户任务中心+运营控制台双视图、跨服务任务案例、SSE 实时推送、SLA 告警 | `unified-task-center-design.md` |
| 新增：资产工作台 | M14 模块：合并生成历史+任务监控、项目树+分类+详情三栏、批量整理/发布/回收站 | `asset-workbench-design.md` |
| 新增：企业工作台完成 | M15 模块：角色化概览、采购预算、统一审批、跨域审计 | `enterprise-workbench-completion-design.md` |
| 新增：独立空白画布 | 无内容项目绑定创建；purpose=experiment；浮动编辑器交互替代右侧面板+底部生成栏 | `standalone-canvas-design.md` |
| 新增：生产 SOP 完成 | M8 补强：双界面（项目控制台+画布质检侧栏）、6 Gate、规则引擎、返工闭环 | `production-sop-completion-design.md` |
| 更新：脚本创作 | M2：三层解耦（源正文→改编脚本→可选分镜）、非强制线性、自适应工作流替代固定向导 | `script-creation-v7-prd.md` |
| 更新：画布工作台 | M6：浮动编辑器交互替代右侧面板+底部生成栏；独立空白画布创建入口 | `canvas-node-floating-editor-design.md` |
| MoSCoW 更新 | 增加 P0 项 F-181~F-186（任务中心/资产工作台/企业工作台/SOP/Agent配置/独立画布）、P1 项 F-187~F-188 | 全部 |

### V1.6 更新（2026-07-02）

| 更新项 | 说明 | 依据 superpowers |
|---|---|---|
| 产品定位扩展 | 增加创作圣经、Agent 协同、交易市场为新支柱 | 全部 |
| 脚本创作重定义 | 线性 10 步→自适应工作流；三种创作模式；`content_projects` 为根对象 | `script-creation-v7-prd.md` |
| 新增：创作圣经 | M10 模块：五大子系统（生态系统/实体/关系/写作指南/连续性）、人工确认、版本隔离 | `script-creation-creative-bible-design.md` |
| 新增：Agent 会话 | M11 模块：项目级 AI 协作者、结构化计划→审批→执行、SSE 事件流 | `agent-session-completion-design.md` |
| 新增：脚本交易市场 | M12 模块：四种许可证、个人/企业购买、双服务财务架构、退款/争议 | `script-trading-market-completion-design.md` |
| 更新：资产管理 | M5：从 mock→完整市场（发现/认领/发布/审批/项目应用） | `ai-asset-market-completion-design.md` |
| 更新：画布工作台 | M6：平台首页、画布项目中心、内容项目归属、浮动编辑器交互 | `platform-home-canvas-center-design.md`、`canvas-node-floating-editor-design.md` |
| 更新：分镜系统 | 双轨→统一专业分镜编辑器（13 维镜头、A/B/C 分层） | `storyboard-professional-editor-redesign.md` |
| 更新：仓库/启动台 | 创作启动台（`/script-gen`）与仓库（`/warehouse`）分离；三轴状态 | `script-creation-warehouse-flow-design.md` |
| MoSCoW 更新 | 增加新 P0 功能项（创作圣经、Agent 会话、交易市场、资产认领、画布中心） | 全部 |

> **注意**：本文档中标注 `[superpowers 更新 V1.6]` 的段落为 V1.6 新增或修改内容；标注 `[superpowers 更新 V1.7]` 的段落为 V1.7 新增或修改内容。
...
6. [功能需求详述](#6-功能需求详述)
   ...
   - [M10：创作圣经](#m10创作圣经) `[superpowers 更新 V1.6]`
   - [M11：Agent 会话](#m11agent-会话项目-ai-协作者) `[superpowers 更新 V1.6]`
   - [M12：脚本交易市场](#m12脚本交易市场) `[superpowers 更新 V1.6]`
   - [M13：资产市场完整功能](#m13资产市场完整功能) `[superpowers 更新 V1.6]`
   - [M14：统一任务事件中心](#m14统一任务事件中心) `[superpowers 更新 V1.7]`
   - [M15：资产生成工作台](#m15资产生成工作台) `[superpowers 更新 V1.7]`
   - [M16：企业工作台（完成）](#m16企业工作台完成) `[superpowers 更新 V1.7]`
   - [M17：Agent 配置中心](#m17agent-配置中心) `[superpowers 更新 V1.7]`
7. [非功能性需求](#7-非功能性需求)
8. [验收标准](#8-验收标准)
9. [风险与依赖](#9-风险与依赖)
...
| 版本周期 | 第一周期（V1.0 → V1.1 → V1.2 → V1.3 → V1.5 → V1.6 → V1.7） |
...
**AI漫剧与视频内容工业化生产工作台** — 通过无限画布、节点系统、资产库、分镜生产、批量生成、工作流模板、Agent 配置中心、任务事件中心、资产工作台与 Agent / Skill 自动执行，完成从剧本到可交付镜头素材包的工业化生产闭环。`[superpowers 更新 V1.7]`
...
| **差异化** | 以「项目→画布→节点→连线→分镜→资产→批量生成→质检→素材包导出→工作流→Agent/Skill」组织完整生产系统，V1.7 新增 Agent 配置中心、任务事件中心、资产工作台三大支柱，竞争力不只在单次生成效果，而在工程化、标准化、可追溯、可复用和可规模化 `[superpowers 更新 V1.7]` |
...
| 🔥 F-162 `[V1.6]` | 创作圣经 | 生态系统规则管理：9类规则CRUD + AI提取候选 + 人工逐字段确认 + 不可变版本快照 | M |
| 🔥 F-163 `[V1.6]` | 创作圣经 | 三层写作指南：项目基线(L1)→角色声音(L2)→单集覆盖(L3)，不可覆盖键保护 | M |
| 🔥 F-164 `[V1.6]` | Agent | Agent持久化会话：数据库存储会话/消息/计划/步骤，SSE实时事件流+断线重连 | M |
| 🔥 F-165 `[V1.6]` | Agent | Agent计划审批执行：结构化计划→用户审批→Tool分级执行(READ/WRITE/BILLABLE) | M |
| 🔥 F-166 `[V1.6]` | 交易 | 脚本挂牌与审核：市场搜索/筛选/预览 + 四种许可证(免费/普通/独家/买断) | M |
| 🔥 F-167 `[V1.6]` | 交易 | 购买与交付：个人直接购买 + 企业采购审批→支付→快照交付→7天结算 | M |
| 🔥 F-168 `[V1.6]` | 资产 | 资产市场完整功能：公共市场发现/认领 + 企业发布审批 + 项目应用/撤消 | M |
| 🔥 F-169 `[V1.6]` | 画布 | 画布项目中心：跨项目画布管理 + 生产准入检查 + 源快照diff + 上游变更通知 | M |
| 🔥 F-170 `[V1.6]` | 分镜 | 专业分镜编辑器：13维镜头编辑 + A/B/C层级版本 + XLSX双向导入导出 | M |
| 🔥 F-171 `[V1.6]` | 平台 | 统一账户架构：3001为账户中心唯一数据源 + `X-Workspace-Id`工作区隔离 | M |

### 5.6 V1.7 新增功能（8项）`[superpowers 更新 V1.7]`

| ID | 模块 | 功能点 | 优先级 |
|------|------|------|:---:|
| 🔥 F-181 `[V1.7]` | 任务中心 | 统一任务中心：用户任务中心+运营控制台双视图；跨服务任务案例(trace_id)；SSE 实时推送；SLA 告警 | M |
| 🔥 F-182 `[V1.7]` | 资产 | 资产工作台：合并生成历史+任务监控；项目树+分类+详情三栏布局；批量整理/发布/回收站 | M |
| 🔥 F-183 `[V1.7]` | 企业 | 企业工作台完成：角色化概览（管理员/部门负责人/成员）；采购预算看板+消耗投影；统一审批（采购/资产发布/项目导出）；跨域审计日志 | M |
| 🔥 F-184 `[V1.7]` | SOP | 生产SOP完成：双界面（项目控制台+画布质检侧栏）；6 Gate 质量门禁；规则引擎可配置；返工闭环（驳回→修复→复检） | M |
| 🔥 F-185 `[V1.7]` | Agent | Agent配置中心 P0：4 系统蓝图(Hook/编剧/分镜/导演)；结构化+高级双编辑模式；版本生命周期（draft→published→deprecated）；项目绑定设置默认 Agent | M |
...
> `[superpowers 更新 V1.7]` 三层解耦架构：**源正文→改编脚本→可选分镜**，非强制线性流程。`content_projects` 为统一根对象，自适应工作流替代固定 10 步向导。用户可在任何阶段进入和退出，不强依赖前置步骤完成。
...
> `[superpowers 更新 V1.7]` 浮动编辑器交互替代右侧面板+底部生成栏，节点编辑以浮动弹窗形式在画布中央呈现，减少视口切换。新增**独立空白画布**创建入口（purpose=experiment），无需绑定内容项目即可创建实验性画布。
...
**核心能力**：
- **五大子系统**：生态系统规则（9 类）、实体设定（角色/背景/势力/地点/物品）、动态关系网络、三层写作指南（项目→角色→单集）、连续性账本
- **人工确认流水线**：AI 提取候选 → 逐字段决策（合并/保留/采用）→ 事务写入，无确认不进入生产
- **不可变版本快照**：确认后生成 `snapshotJson`，所有 AI 生成引用特定圣经版本
- **变更影响报告**：上游圣经变更 → 生成下游受影响产出物清单 + 建议操作

**页面入口**：内容项目详情 →「创作圣经」Tab → 子系统卡片（生态规则、实体设定、写作指南、连续性快照）
...
**核心能力**：
- **持久化会话**：绑定 `project_id`，浏览器刷新/重启不丢失
- **结构化计划→审批→执行**：AI 先出计划（含步骤、风险、成本估算），用户审批后执行
- **SSE 实时事件流**：执行进度实时推送，断线重连不丢事件
- **写作集成**：预览 Patch diff → 审批 → 应用（乐观锁保护）
- **画布集成**：读取/创建/更新节点、创建生成任务（结果附加为候选，不自动采用）

**三栏布局**：左侧会话列表 → 中间消息流（含计划审批卡片）→ 右侧上下文面板/执行时间线
...
**核心能力**：
- **四种许可证**：免费/普通（可重复）/独家（单次销售）/买断（完整权利转移）
- **个人/企业购买**：企业成员提交采购申请 → 审批人审核预算 → 确认支付
- **快照交付**：购买时冻结价格、条款和内容，卖家后续编辑不影响已购副本
- **7 天结算**：卖家收入冻结 7 天后解冻，期间支持退款争议
- **双服务财务**：8080 交易业务 + 3001 钱包账本（不可变双记账条目）

**页面入口**：顶部导航 →「交易市场」→ 搜索/筛选/预览/购买
...
**核心能力**：
- **公共市场**：搜索/分类/标签/排序发现，一键认领（幂等）
- **工作区库**：个人/企业资产管理和版本管理
- **企业审批**：企业成员发布 → 管理员审批 → 自动创建 Market Listing
- **项目应用**：风格模型→合并 style_config、角色→加入项目角色引用、场景→加入场景引用、提示词→复制到项目提示词库
- **应用撤消**：SHA-256 安全 Token + `project_row_version` 校验

---

### M14：统一任务事件中心 `[superpowers 更新 V1.7]`

> 优先级：P0 | 版本：V1.7 | 详见：`docs/superpowers/specs/2026-07-04-unified-task-center-design.md`

**用户故事**：作为创作者/运营，我希望在一个统一界面查看当前 Workspace 内所有生成和交易任务的状态、时间线和费用，接收实时推送，并对异常任务执行操作。

**核心能力**：

- **US-M14-01：用户查看所有任务** — 在 `/task-center` 统一查看当前 Workspace 内所有生成+交易任务，支持按类型/状态/时间筛选和搜索
  - **AC**：页面展示任务列表（类型图标+名称+状态+创建时间+进度）；支持筛选（全部/生成/交易/导出）和状态过滤（排队中/执行中/成功/失败/已取消）；分页加载，每页 20 条
  - **API**：`GET /task-center/tasks?workspace_id=&type=&status=&page=`
  - **UI**：左侧筛选栏 + 右侧任务卡片列表；每个卡片显示任务类型徽标、进度条、耗时、费用

- **US-M14-02：任务详情时间线** — 查看任务阶段时间线、执行尝试、费用构成
  - **AC**：点击任务卡片展开详情面板，展示阶段时间线（提交→排队→执行→回调→完成）；显示每次重试记录和失败原因；费用分解（模型调用费/存储费/其他）
  - **API**：`GET /task-center/tasks/:taskId/timeline`
  - **UI**：垂直时间线 UI，每个阶段显示状态图标+时间戳+耗时；费用饼图或条形分解

- **US-M14-03：SSE 实时更新** — 任务状态变化实时推送，无需手动刷新
  - **AC**：页面建立 SSE 连接（`/task-center/events?workspace_id=`）；任务状态变更时列表项实时更新（排队→执行中→成功/失败）；断线 30 秒内自动重连
  - **API**：`GET /task-center/events` (SSE)
  - **UI**：列表项状态徽标无闪烁切换；新任务到达列表顶部带有进入动画

- **US-M14-04：任务操作** — 取消/重试/继续付款等命令
  - **AC**：排队中或执行中任务可取消（确认弹窗）；失败任务可重试（保留原参数，允许修改）；支付中断任务可继续付款
  - **API**：`POST /task-center/tasks/:taskId/cancel` `POST /task-center/tasks/:taskId/retry`
  - **UI**：每行操作按钮（取消/重试/继续付款），根据任务状态动态显示

- **US-M14-05：运营控制台** — 管理员查看全局态势、异常队列、对账差异
  - **AC**：运营控制台展示全局任务量/成功率/平均耗时仪表板；异常队列（超时/失败突增）红色高亮；供应商对账差异列表
  - **API**：`GET /admin/task-center/dashboard` `GET /admin/task-center/anomalies`
  - **UI**：仪表板布局（顶部指标卡 + 中间趋势图 + 底部异常列表）

**页面入口**：顶部导航 →「任务中心」→ 用户视图（默认）/ 运营控制台（管理员切换）

---

### M15：资产生成工作台 `[superpowers 更新 V1.7]`

> 优先级：P0 | 版本：V1.7 | 详见：`docs/superpowers/specs/2026-07-04-asset-workbench-design.md`

**用户故事**：作为创作者，我希望在一个统一的工作台浏览、分类、预览和管理所有历史生成资产，并快速将资产发送到画布继续创作。

**核心能力**：

- **US-M15-01：项目树浏览** — 左侧项目树+未归档集合，按内容项目组织资产
  - **AC**：左侧展示项目树（按 Workspace→Project 层级）；每个项目节点展开显示资产分类；顶部「未归档」集合收纳不属于任何项目的资产
  - **API**：`GET /asset-workbench/projects-tree?workspace_id=`
  - **UI**：左侧可折叠树形导航，项目名+资产数量徽标

- **US-M15-02：分类资产列表** — 按角色/场景/道具/分镜/语音/音乐分类查看
  - **AC**：中间区域按分类 Tab 展示资产列表（全部/角色/场景/道具/分镜图/语音/音乐）；每个资产卡片显示缩略图+名称+创建时间+成熟度；支持网格/列表视图切换
  - **API**：`GET /asset-workbench/assets?project_id=&category=&page=`
  - **UI**：分类 Tab 栏 + 资产网格（缩略图卡片）+ 排序/视图切换工具栏

- **US-M15-03：资产详情** — 右侧抽屉显示详情和操作
  - **AC**：点击资产卡片打开右侧详情抽屉；展示大预览图、生成参数（Prompt/模型/种子/分辨率）、使用记录（被哪些画布节点引用）、版本历史
  - **API**：`GET /asset-workbench/assets/:assetId`
  - **UI**：右侧抽屉面板，包含预览区+参数区+使用记录时间线+操作按钮

- **US-M15-04：发送到画布** — 选择目标画布+放置位置
  - **AC**：点击「发送到画布」→ 弹出画布选择器→ 选择目标画布项目→ 可选放置坐标→ 确认后在画布创建对应节点
  - **API**：`POST /asset-workbench/assets/:assetId/send-to-canvas` (body: `canvas_project_id`, `position_x`, `position_y`)
  - **UI**：画布选择弹窗（最近画布列表+搜索）+ 坐标预览小地图

- **US-M15-05：批量操作** — 批量归档/删除/标签
  - **AC**：支持多选资产（Shift/Ctrl+点击）→ 批量工具栏出现→ 批量归档/删除/添加标签；批量删除需二次确认+展示受影响资产数量
  - **API**：`POST /asset-workbench/assets/batch` (body: `asset_ids`, `action`)
  - **UI**：多选模式下每张卡片出现复选框；底部浮现批量操作工具栏

- **US-M15-06：重新生成** — 保留原始参数+允许修改
  - **AC**：资产详情中点击「重新生成」→ 弹出参数面板（预填原始 Prompt/模型/种子）→ 用户可修改→ 提交生成任务→ 结果作为新资产版本
  - **API**：`POST /asset-workbench/assets/:assetId/regenerate`
  - **UI**：参数编辑弹窗 + 积分预估 + 提交按钮

**页面入口**：顶部导航 →「资产工作台」→ 三栏布局（项目树+资产列表+详情抽屉）
...
- **US-M16-01：角色化概览** — 管理员/部门负责人/成员看到不同内容
  - **AC**：管理员看到全企业数据概览（成员数/项目数/消耗/预算）；部门负责人看到本部门数据+成员项目列表；普通成员看到个人数据+团队项目入口
  - **API**：`GET /enterprise/dashboard?role=`
  - **UI**：仪表板卡片布局，不同角色展示不同的指标卡片组合

- **US-M16-02：组织管理** — 部门树、成员列表、邀请、角色分配
  - **AC**：左侧部门树（可展开/折叠/拖拽调整）；成员列表（头像/姓名/角色/部门/最后活跃）；邀请成员（邮箱/手机号+选择部门+选择角色）；角色分配（管理员/部门负责人/成员/只读）
  - **API**：`GET /enterprise/org/departments` `POST /enterprise/org/members/invite` `PUT /enterprise/org/members/:memberId/role`
  - **UI**：左侧部门树+右侧成员表格；邀请弹窗表单

- **US-M16-03：采购预算** — 月度预算看板、消耗投影
  - **AC**：月度预算总额+已消耗+剩余+消耗趋势图；按部门/项目拆分消耗明细；基于历史消耗的月底投影；预算告警阈值设置
  - **API**：`GET /enterprise/budget?month=` `GET /enterprise/budget/projection`
  - **UI**：预算仪表板（环形图/趋势折线图/部门消耗排行表）

- **US-M16-04：统一审批** — 采购/资产发布/项目导出三类审批
  - **AC**：统一审批列表（待审批/已审批/已驳回）；采购审批（预算校验+审批链）；资产发布审批（内容合规+版权校验）；项目导出审批（导出范围+水印设置）；审批操作（通过/驳回/加签）
  - **API**：`GET /enterprise/approvals?status=` `POST /enterprise/approvals/:approvalId/approve` `POST /enterprise/approvals/:approvalId/reject`
  - **UI**：审批列表+详情面板；通过/驳回操作按钮+理由输入框

- **US-M16-05：Workspace 切换** — 个人+企业多 workspace 切换
  - **AC**：顶部导航显示当前 Workspace 名称；点击展开下拉菜单（个人 Workspace + 所属企业 Workspace 列表）；切换后全局上下文刷新（项目列表/资产/任务中心等）
  - **API**：`GET /workspaces` `POST /workspaces/switch` (设置 `X-Workspace-Id` header)
  - **UI**：Header 区域 Workspace 下拉选择器+当前 Workspace 标识

**页面入口**：顶部导航 →「企业工作台」→ 角色化仪表板 → 组织/预算/审批子页面

---

### M17：Agent 配置中心 `[superpowers 更新 V1.7]`

> 优先级：P0/P1 | 版本：V1.7 | 详见：`docs/superpowers/specs/2026-07-04-agent-config-center-design.md`

**用户故事**：作为 Agent 开发者/项目管理员，我希望在一个配置中心浏览系统蓝图、创建和编辑 Agent 配置、试跑验证效果、管理版本生命周期，并将 Agent 绑定到项目。

**核心能力**：

- **US-M17-01：浏览蓝图** — 查看 4 个系统蓝图（Hook/编剧/分镜/导演）
  - **AC**：蓝图列表展示 4 个系统预置 Agent 蓝图；每个蓝图卡片显示名称/描述/适用阶段/默认参数概览；蓝图只读，需复制后才可编辑
  - **API**：`GET /agent-config/blueprints`
  - **UI**：蓝图卡片网格 + 详情预览面板

- **US-M17-02：创建 Agent** — 选蓝图+命名+配置参数
  - **AC**：选择蓝图→填写 Agent 名称（唯一）→ 进入配置编辑页；支持从蓝图复制或从零创建；保存后生成唯一 Agent ID
  - **API**：`POST /agent-config/agents` (body: `blueprint_id`, `name`, `description`)
  - **UI**：创建向导（蓝图选择→基本信息→配置编辑）

- **US-M17-03：编辑 Prompt** — 结构化参数或高级模式
  - **AC**：结构化模式：表单化编辑 system_prompt/user_prompt/temperature/max_tokens/stop_sequences 等参数；高级模式：YAML/JSON 原始编辑器，带语法高亮和校验；两种模式可实时切换，数据双向同步
  - **API**：`PUT /agent-config/agents/:agentId/config`
  - **UI**：左右分栏（结构化表单 + 实时预览/校验结果）

- **US-M17-04：试跑 Agent** — 输入测试→查看输出预览+费用
  - **AC**：试跑面板输入测试文本→ 选择模型→ 点击运行→ 实时流式输出预览；完成后展示 Token 用量和预估费用；支持保存试跑结果为案例
  - **API**：`POST /agent-config/agents/:agentId/test-run` (SSE 流式响应)
  - **UI**：左侧测试输入+右侧流式输出；底部 Token 计数+费用条

- **US-M17-05：发布/回滚** — 版本生命周期管理
  - **AC**：版本状态流转：draft→published→deprecated→archived；发布时自动创建版本快照；回滚选择历史版本→预览差异→确认回滚→生成新版本号
  - **API**：`POST /agent-config/agents/:agentId/publish` `POST /agent-config/agents/:agentId/rollback`
  - **UI**：版本时间线（各版本卡片+状态徽标+差异预览）

- **US-M17-06：项目绑定** — 项目管理员设置默认 Agent
  - **AC**：项目设置→ Agent 配置→ 为每个阶段（Hook/编剧/分镜/导演）选择默认 Agent；支持选择系统蓝图或自定义 Agent；设置后项目中该阶段的 AI 调用自动使用绑定的 Agent
  - **API**：`PUT /agent-config/projects/:projectId/bindings`
  - **UI**：阶段-Agent 配对选择器（下拉菜单+Agent 预览卡片）

- **US-M17-07：试跑 A/B 对比（P1）** — 同一输入同时运行两个 Agent 版本
  - **AC**：选择 Agent A 和 Agent B→ 输入相同测试文本→ 并排展示两个输出→ 对比 Token 用量和费用；支持投票标记偏好
  - **API**：`POST /agent-config/agents/:agentId/compare`
  - **UI**：并排对比视图（A|B），差异高亮+指标对比表

- **US-M17-08：TEAM 可见性（P1）** — Agent 配置在企业/团队内共享
  - **AC**：Agent 可见性设置：PRIVATE（仅自己）/TEAM（企业内）/PUBLIC（公开市场）；TEAM 级别需管理员审批
  - **API**：`PUT /agent-config/agents/:agentId/visibility`
  - **UI**：可见性下拉选择器+审批状态指示

**页面入口**：顶部导航 →「Agent 配置」→ 蓝图浏览/我的 Agent/项目绑定 三个 Tab
