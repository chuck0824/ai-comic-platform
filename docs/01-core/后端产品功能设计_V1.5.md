...
| V1.3 | 以 AI 视频创作自由画布 PRD 为基础，对 `canvas-svc` 进行系统化增强：无限画布、节点体系、节点下游菜单、全能参考、多副本生成、角色/场景/道具资产联动、时间线合成、Agent/Skill 自动执行、OpenAPI、Adapter、SOP 质检闭环 |
| V1.5 | 以“AI 漫剧与视频内容工业化生产工作台”为最终定位，后端必须支撑项目、画布、节点、连线、分镜、资产、批量生成、时间线、导出、工作流、Agent/Skill 的完整生产闭环；P0 新增双击建节点、拖入素材建节点、脚本全屏编辑、批量生图/视频、图片拉出视频节点、首尾帧视频、全能参考、多副本并行、资产历史生成、视频合成时间线、Agent 会话/Skill 调用 |
...
| `storyboard_shots` | 分镜生产单元 | `id`、`storyboard_id`、`project_id`、`shot_no`、`scene_no`、`duration`、`shot_size`、`camera_motion`、`visual_description`、`characters`、`scene_asset_id`、`image_prompt`、`video_prompt`、`image_status`、`video_status` |
...
| E2E 测试 | 创建脚本节点 -> 拆分分镜 -> 批量生图 -> 图生视频 -> 加入时间线 -> 导出 |
...
| **剧本生成服务** | `script-gen-svc` | 剧本资产生成编排、样章/故事圣经/章节/分镜Master、Prompt模板 | 剧本、分镜、AI编排 | P0 |
...
| **画布视频工作台服务** | `canvas-svc` | 画布编排、分镜管理、时间轴、视频合成、导出 | 画布、分镜、视频、导出 | P0 |
| **工业化生产SOP服务** | `sop-svc` | 生产准入、审计返工、版本锁定、产能估算 | SOP、审计、权限 | P1 |
| **通知与消息服务** | `notify-svc` | 站内通知、邮件、短信、浏览器推送 | 通知、消息 | P0 |
| **Agent与Skill服务** | `agent-svc` | AI导演、Skill执行、任务编排、第三方Agent接入、工具调用 | Agent、Skill、OpenAPI、自动化 | P1 |
...
> **服务ID**：`script-gen-svc`  
> **优先级**：P0  
> **职责**：AI剧本资产生成编排、6步向导引擎、故事圣经/章节/分镜Master生成、A/B/C三档分镜升级、与LLM/图像模型交互
...
→ scripts / script_episodes / script_versions
→ storyboard_master
...
| 完整保存 | 保存剧本时必须保存章节、故事圣经、大纲、分镜、投流素材，并创建初始版本 |
| 上传解析 | 上传接口只创建解析任务，前端通过任务ID轮询真实状态 |
| 分镜Master | 剧本侧分镜是Master；画布导入时生成生产快照 |
...
> **服务ID**：`canvas-svc`  
> **优先级**：P0  
> **职责**：无限画布状态管理、节点编排、剧本/分镜/图片/视频/音频节点管理、角色/场景/道具资产联动、视频生成、时间线合成、成片导出、工作流模板、Agent 执行落盘。

### 9.0 本次迭代定位

V1.3 不推翻原第一周期的画布与视频工作台设计，而是在原有“分镜卡片 + 时间轴 + 导出”的基础上，将画布能力升级为：
...
| POST | `/api/v1/canvas/projects/:id/video-nodes/:nodeId/trim` | 视频裁剪 | V1.3 |
...
| PUT | `/api/v1/canvas/projects/:id/timeline` | 更新时间线 | V1.0 |
| POST | `/api/v1/canvas/projects/:id/timeline/add-node` | 将节点加入时间线 | V1.3 |
...
| 页面区域 | 前端功能 | 后端能力 |
|---|---|---|
| 顶部工具栏 | 返回项目、项目名、撤销、重做、自动排版、分享、导出、算力 | 项目CRUD、快照、操作日志、导出任务、积分预估 |
| 左侧节点栏 | 文本、脚本、图片、视频、音频、角色、场景、道具、合成节点 | 节点类型注册、节点创建、资产引用 |
...
| 底部时间线 | 视频轨、音频轨、字幕轨、转场 | timeline JSON、compose任务、FFmpeg渲染 |
...
| 导出视频 | 创建 compose/export 任务，异步通知用户 |
...
| `project_id` | VARCHAR(50) | 项目ID |
| `user_id` | BIGINT | 操作人 |
| `operation_type` | ENUM('create_node','update_node','delete_node','move_node','create_edge','delete_edge','group','ungroup','set_main_version','timeline_update') | 操作类型 |
| `payload` | JSON | 操作内容 |
...
| `type` | ENUM('text','script','image','video','audio','character','scene','prop','compose','workflow','agent') | 节点类型 |
...
| `video_generated` | 视频已生成 | 加入时间线、重新生成 |
...
| 视频剪辑 | `video_trim` | 原视频 + 入点/出点 | 剪辑片段 |
...
| POST | `/api/v1/canvas/projects/:id/audio-nodes/:nodeId/add-to-timeline` | 加入时间线 |
...
```json
{
  "compose_node_id": "COMPOSE_001",
  "timeline": {
    "video_tracks": [
      {
        "track_id": "V1",
        "clips": [
          { "node_id": "VIDEO_001", "asset_id": "VID_001", "start_ms": 0, "duration_ms": 5000 }
        ]
      }
    ],
    "audio_tracks": [
      {
        ...
        "type": "voice",
        "clips": [
          { "node_id": "AUDIO_001", "asset_id": "AUD_001", "start_ms": 0, "volume": 1.0 }
        ]
      }
    ],
    "subtitle_tracks": [
      {
        "track_id": "S1",
        "clips": [
          { "text": "这一次，我不会再输。", "start_ms": 1200, "end_ms": 3200 }
        ]
      }
    ],
    "transition_track": [],
...
| 画幅 | 分辨率 | 码率 | 编码 | 适用平台 |
|---|---|---|---|---|
| 9:16 | 720×1280 | 2Mbps | H.264 | 抖音/快手标准 |
| 9:16 | 1080×1920 | 4Mbps | H.264 | 抖音/快手高清 |
| 16:9 | 1280×720 | 2Mbps | H.264 | B站/YouTube标准 |
| 16:9 | 1920×1080 | 6Mbps | H.265 | B站/YouTube高清 |
...
| `description` | TEXT | 描述 |
| `category` | ENUM('script_to_storyboard','character_design','scene_design','image_to_video','compose','custom') | 分类 |
...
| 模板 | 节点链路 |
|---|---|
| 小说转漫剧 | 文本节点 → 脚本节点 → 图片节点 → 视频节点 → 合成节点 |
| 角色设定 | 文本节点 → 角色节点 → 图片节点 → 三视图 / 表情图 |
| 场景设定 | 文本节点 → 场景节点 → 多机位图片节点 |
| 图生视频 | 图片节点 → 视频节点 → 高清节点 |
| 广告片 | 产品图 → 脚本节点 → 分镜图 → 视频节点 → 合成节点 |
| 成片合成 | 多视频节点 → 音频节点 → 字幕节点 → 合成节点 |
...
| `canvas.add_to_timeline` | `POST /timeline/add-node` |
...
| 导出视频 | `canvas_export_completed` | 成片导出 |
...
| 节点系统 | 支持 text/script/image/video/audio/character/scene/compose 节点创建和编辑 |
...
| 视频结果是否匹配画幅 | video metadata | 禁止加入时间线 |
| 字幕是否覆盖全部台词 | subtitle_track | 阻止导出 |
| BGM/SFX是否过大 | audio volume | 给出混音建议 |
...
| 合成失败 | 检查素材编码、转码、重建时间线 | FFmpeg连续失败 |
...
| 节点体系 | text/script/image/video/audio/character/scene/prop/compose/workflow/agent | P0 |
...
| 时间线合成 | `canvas-svc` | 9.11 时间线与合成节点 |
...
| 包5：时间线合成 | timeline、compose、export、字幕音轨 | 2-3周 |
