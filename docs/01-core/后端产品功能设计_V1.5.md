...
| **Canvas 生产内核重构** `[superpowers 更新 V1.8]` | 节点类型 11→6 收缩（text/image/video/audio/script/director）；EXPLORATION/PRODUCTION 双模式；ShotWorkUnit 镜头生产容器+5 Gate；12 类类型化端口（含方向与角色）；Three.js 导演台（DirectorDraft/DirectorRevision 版本流+乐观锁）；CapabilityRequest→ModelAdapter→Preview→Confirmation→Snapshot→Task→Attempt→Candidate→Quality→Adoption→Delivery 生产管线；`generation_request_snapshots`/`generation_candidates`/`shot_adoptions`/`delivery_manifests` 等 13 张新表；EDL/FCPXML 外部剪辑交接；Feature Flag 五开关（CANVAS_KERNEL_V2/TYPED_PORTS/DIRECTOR_V2/MODEL_ADAPTER_V2/QUALITY_DELIVERY_V2）；绞杀迁移策略（审计→影子读→单画布升级→灰度）；R0-R5 五阶段交付 | `canvas-production-kernel-completion-design.md` + R0-R4 实施计划 |

> **注意**：本文档中标注 `[superpowers 更新 V1.8]` 的段落为 2026-07-05 新增或修改内容（Canvas 生产内核）。标注 `[superpowers 更新 V1.7]` 的段落为 2026-07-04 新增或修改内容。标注 `[superpowers 更新]` 的段落为 07-02 之前的更新。未标注的原有内容保持不变，作为历史基线参考。
...
| V1.8 `[superpowers 更新 V1.8]` | Canvas 生产内核重构：节点类型从 11 种收敛为 6 种标准类型（text/image/video/audio/script/director），角色/场景/道具/prompt/参考/工作流不再作为画布节点类型，转为领域资源通过类型化端口引用；新增 EXPLORATION/PRODUCTION 双模式、ShotWorkUnit 镜头生产容器、12 类类型化端口、Three.js 导演台 Draft/Revision 版本流、CapabilityRequest→ModelAdapter 生产管线、质量报告、正式采用（ShotAdoption）、交付清单（DeliveryManifest）、EDL/FCPXML 外部剪辑交接、绞杀迁移策略、Feature Flag 五开关、R0-R5 五阶段交付 |
...
> 
> 内容项目模块（V7）新增了 24 张表（M0-M4）、9 个 Controller、23 个 Service。所有项目端点已接入 `ProjectAccessService` 细粒度鉴权。AI 响应解析统一使用 `AiResponseParser` 共享工具类。详见 `docs/superpowers/specs/script-creation/2026-06-29-script-creation-v7-prd.md`。
...
> **详细设计见**：`docs/superpowers/specs/workbench/2026-06-28-unified-account-model-billing-design.md`
...
| 导演台 | PUT | `/api/v1/canvas/projects/:id/director-desk/:nodeId` `[已废弃-superpowers V1.8]` | 旧 DOM 导演台保存。已由 DirectorScene Draft/Revision API 替代，详见 Section 9.19.4 | V1.5 |
...
| 左侧节点栏 | 文本、脚本、图片、视频、音频、导演台节点 `[superpowers 更新 V1.8]`（原角色/场景/道具节点 `[已废弃]`，转为领域资源通过类型化端口引用） | 节点类型注册、节点创建、资产引用 |
...
> **[superpowers 更新 V1.8 — 冲突裁决]**：原口径为 11 种节点类型（含 character/scene/prop/workflow/agent）。根据 `canvas-production-kernel-completion-design.md` 第 1 节产品边界和第 4.3 节节点与端口，**节点类型严格保持 text/image/video/audio/script/director 六类**。角色、场景、道具、提示词、参考和工作流不再作为画布节点类型——角色/场景/道具转为领域资源通过类型化端口（`character`/`scene`/`prop` 端口载荷）引用；工作流转为 ShotWorkUnit 内的生产模板。旧类型保留兼容读取，新链路只写六类。

| 节点类型 | 类型值 | 主要用途 | 状态 |
|---|---|---|:---:|
| 文本节点 | `text` | 剧本片段、Prompt、旁白、说明 | ✅ 保留 |
| 脚本节点 | `script` | 分镜表、镜头表、批量生成入口 | ✅ 保留 |
| 图片节点 | `image` | 角色图、场景图、分镜图、产品图 | ✅ 保留 |
| 视频节点 | `video` | 文生视频、图生视频、首尾帧视频 | ✅ 保留 |
| 音频节点 | `audio` | TTS、BGM、音效、角色音色 | ✅ 保留 |
| 导演台节点 | `director` | Three.js 3D 构图、Draft/Revision 版本流、机位/动作/灯光预设 | ✅ 新增 |
| ~~角色节点~~ | ~~`character`~~ `[已废弃-superpowers V1.8]` | 角色资产不再作为节点类型；通过 `character` 端口载荷引用角色资产 | ❌ |
| ~~场景节点~~ | ~~`scene`~~ `[已废弃-superpowers V1.8]` | 场景资产不再作为节点类型；通过 `scene` 端口载荷引用场景资产 | ❌ |
| ~~道具节点~~ | ~~`prop`~~ `[已废弃-superpowers V1.8]` | 道具资产不再作为节点类型；通过 `prop` 端口载荷引用道具资产 | ❌ |
| ~~工作流节点~~ | ~~`workflow`~~ `[已废弃-superpowers V1.8]` | 工作流不再作为节点类型；ShotWorkUnit 内生产模板替代 | ❌ |
| ~~Agent节点~~ | ~~`agent`~~ `[已废弃-superpowers V1.8]` | Agent 不再作为节点类型；Agent 通过统一 AgentSessionService facade 与节点交互 | ❌ |

> **迁移说明**：旧 `reference` 节点中携带导演 JSON 数据的自动映射为 `director`；其他 `reference` 根据媒体类型进入迁移向导。旧类型保留只读兼容，详见 `canvas-production-kernel-completion-design.md` Section 12。
...
| `type` | ENUM('text','script','image','video','audio','director') `[superpowers 更新 V1.8]` | 节点类型（6 种标准类型）。旧值 `character`/`scene`/`prop`/`workflow`/`agent`/`reference` 保留兼容读取，新链路只写六类 |
...
| 导演台状态 | `canvas_nodes.data.director_desk` `[已废弃-superpowers V1.8]` | `objects`、`groups`、`cameras`、`screenshots`、`panorama_assets`、`shortcuts` | DOM 原型 3D 空间。已被 `director_drafts`/`director_revisions` 替代，详见 Section 9.19.4 |
...
| ~~音色克隆~~ | ~~`audio`~~ `[已废弃-superpowers V1.8]` | ~~`voice_clone`~~ — 音色克隆功能移交音频资产域，不在 Canvas 内提供 |
| ~~音频截取/变速~~ | ~~`audio`~~ `[已废弃-superpowers V1.8]` | 音频截取和变速不在 Canvas 产品边界内，由外部音频工具处理 |
...
### 9.11 镜头采用版本与素材包交付 `[已废弃-superpowers V1.8 — 由 Section 9.19.6-9.19.7 替代]`

> **[superpowers 更新 V1.8]**：本节描述的采用版本与素材包交付已被 ShotAdoption + DeliveryManifest + EDL/FCPXML 模型替代。旧 API 保留兼容读取，新链路使用 Section 9.19.7 的交付清单体系。原始内容保留如下作为历史参考。
...
### 9.19 Canvas 生产内核 V1.8 `[superpowers 更新 V1.8]`

> 本节内容基于 `docs/superpowers/specs/canvas/2026-07-05-canvas-production-kernel-completion-design.md` 及 R0-R4 实施计划。V1.8 将 Canvas 从"画布页面接口"升级为完整的生产内核，引入双模式、ShotWorkUnit、类型化端口、导演台版本流、模型适配器管线、质量报告和交付清单。

#### 9.19.1 CanvasProject 双模式

`canvas_projects` 新增字段：

| 字段 | 类型 | 说明 |
|---|---|---|
| `mode` | ENUM('EXPLORATION','PRODUCTION') | 画布模式，默认 EXPLORATION |
| `schema_version` | INT | 数据 schema 版本，旧画布为 1，升级后为 2 |
| `migration_status` | ENUM('NOT_AUDITED','AUTO_READY','NEEDS_CONFIRMATION','UPGRADED','FAILED') | 迁移状态 |
| `content_project_id` | BIGINT | 绑定的内容项目 ID（PRODUCTION 模式必填） |
| `content_unit_id` | BIGINT | 绑定的生产单元 ID |
| `content_version_id` | BIGINT | 绑定的内容版本 ID |
...
探索画布转正式生产时创建绑定 revision，不覆盖原探索数据。

#### 9.19.2 ShotWorkUnit（镜头工作单元）

ShotWorkUnit 是画布内的镜头生产容器，**不是新增节点类型**。一个 CanvasProject 包含多个 ShotWorkUnit。
...
|---|---|---|
| `id` | BIGINT PK | 自增主键 |
| `uuid` | VARCHAR(36) UNIQUE | 业务标识 |
| `project_id` | BIGINT FK | 所属画布项目 |
| `mode` | VARCHAR(16) | EXPLORATION / PRODUCTION |
| `provisional_shot_id` | VARCHAR(64) | 探索模式临时镜头 ID |
| `source_shot_id` | BIGINT | PRODUCTION 模式绑定的源镜头 ID |
| `source_shot_revision` | INT | 源镜头 revision |
| `duration_ms` | INT | 时长（毫秒） |
| `fps` | INT | 帧率（1-120） |
| `aspect_ratio` | VARCHAR(16) | 画幅（如 16:9） |
| `row_version` | INT | 乐观锁版本号 |
| `created_at` | DATETIME | 创建时间 |
| `updated_at` | DATETIME | 更新时间 |
...
- 镜头身份、画幅、FPS 和基础时长
- 六类节点及类型化端口连线
- **五个生产 Gate**：输入齐套、费用确认、生成完成、质检完成、正式采用
- 可选的 DirectorScene
- 候选、质量报告和正式采用记录
- 交付清单引用
...
> 原有"out → in"通用连线模式已被类型化端口替代。创建连线时后端校验端口兼容性，旧连线只读投影。

端口契约版本 `canvas-ports-v1`，定义 12 种端口载荷类型：

| 端口载荷 | 方向 | 说明 | 可连接的源/目标节点类型 |
|---|---|---|---|
| `text` | INPUT/OUTPUT | 文本/Prompt 内容 | text, script ↔ image, video, audio |
| `shot` | INPUT | 镜头引用（不是节点类型） | script → ShotWorkUnit |
| `character` | INPUT | 角色资产引用 | image, video, director |
| `scene` | INPUT | 场景资产引用 | image, video, director |
| `prop` | INPUT | 道具资产引用 | image, video, director |
| `image_ref` | INPUT/OUTPUT | 图片参考 | image → video, director |
| `motion_ref` | INPUT | 动作参考 | video, director |
| `camera_ref` | INPUT | 摄影机参考 | video, director |
| `audio_ref` | INPUT | 音频参考 | video |
| `director_package` | OUTPUT | 导演包（冻结 revision + 资产引用） | director → video |
| `video_candidate` | OUTPUT | 视频候选结果 | video → quality |
| `quality_report` | OUTPUT | 质量报告 | quality → adoption |

**兼容规则**：`CanvasPortRegistry` 后端统一提供 `canConnect(sourceType, sourcePort, targetType, targetPort)` 校验。无法推断的旧连线标记 `NEEDS_CONFIRMATION`，不静默删除或自动执行。详见 `canvas-production-kernel-completion-design.md` Section 4.3。

`canvas_edges` 新增字段：

| 字段 | 类型 | 说明 |
|---|---|---|
| `port_contract_version` | VARCHAR(32) | 端口契约版本，默认 `canvas-ports-v1` |
| `migration_status` | VARCHAR(32) | AUTO / NEEDS_CONFIRMATION / MANUAL |
| `validation_result` | JSON | 校验摘要 |

#### 9.19.4 DirectorScene 版本流

一个 ShotWorkUnit 最多拥有一个逻辑 DirectorScene。多个视频节点可以引用不同 DirectorRevision 或复用同一 revision。

新增表：

| 表 | 用途 |
|---|---|
| `director_scenes` | 导演场景，一个 ShotWorkUnit 最多一条记录 |
| `director_drafts` | 唯一可编辑草稿，使用 `row_version` 乐观锁 |
| `director_revisions` | 冻结产生的不可变版本，保存规范化 JSON、哈希和资产引用 |
| `director_revision_assets` | revision 引用的资产版本快照 |

**关键约束**：
- 草稿通过 `If-Match` / ETag 乐观锁更新，冲突返回 409 + 差异摘要
- 冻结前必须校验：资产缺失、动作重叠、时间越界、越轴风险、相机穿模、参考职责冲突和模型能力超限
- 冻结创建不可变 revision（哈希稳定），不改变在途任务
- 从冻结 revision 继续编辑时派生新草稿
- 预演、生成和质检均创建独立 Task，不改变 revision 不可变性

**坐标协议**：领域协议 `RH_Y_UP_METERS`（右手坐标、Y-up、米制）。Three.js 直接消费。Blender Worker 入口将领域坐标转为 Blender Z-up，输出时转回。持久化旋转使用归一化 Quaternion。

**时间协议**：半开区间 `[0, duration_ms)`。`frame_count = ceil(duration_ms / 1000 * fps)`。关键帧保存 `time_ms`（事实值）和派生 `frame_index`。

#### 9.19.5 生成管线：CapabilityRequest → Adapter → Snapshot → Task → Candidate

> 原有"节点直接调用模型"路径 `[已废弃-superpowers V1.8]`，替换为以下管线：

```text
节点意图
→ CapabilityCompiler 编译为模型无关 CapabilityRequest
→ ModelAdapter 版本化适配预览（推荐模型、费用、素材职责、警告）
→ 用户确认模型与费用
...
| 表 | 用途 |
|---|---|
| `generation_request_snapshots` | 不可变请求快照：`payload_json`、`payload_hash`（SHA-256）、`resolved_model_id`、`adapter_version`、`estimated_credits` |
| `generation_task_attempts` | 每次供应商调用创建一条 attempt：`task_id`、`attempt_no`、`provider_request_id`、`status`、`error_code`。不可覆盖旧 attempt |
| `generation_candidates` | 候选结果：`request_snapshot_id`、`task_attempt_id`、`output_asset_version_id`、`model_id`、`seed`、`actual_credits` |

> **废弃说明**：`GenerationVariant` 保留为旧接口兼容投影；新链路只写 `GenerationCandidate`，不双写两套候选事实。

**CapabilityCompiler**（Canvas 侧）：将节点、资产和导演语义编译为模型无关 `CapabilityRequest`，包含生成模式、时长、画幅、质量档位、成本偏好和语义化参考角色（identity/scene/composition/audio_timing）。

**ModelAdapter**（版本化）：负责能力限制、素材职责分配、Prompt 引用、参数转换和响应规范化。Seedance Adapter 在供应商 Gate（G0）通过前只能使用 Provider Sandbox。

**预览与确认流程**：
1. POST `/api/v1/canvas/nodes/{nodeId}/model-requests/preview` — 返回推荐模型、费用估算、素材职责、被裁剪输入和警告
2. 用户确认后 POST `/api/v1/canvas/nodes/{nodeId}/model-requests` — 冻结请求快照、扣减积分、创建任务
3. 确认后模型不可用必须重新预览并再次确认，禁止静默切换模型或费用

#### 9.19.6 质量报告

| 表 | 用途 |
|---|---|
| `canvas_quality_reports` | 候选质量报告：`candidate_id`、`overall_status`（PASS/WARN/BLOCK）、`policy_version` |
| `canvas_quality_issues` | 具体质量问题：`start_ms`、`end_ms`、`source_node_id`、`source_track_id`、`dimension`、`severity`、`suggested_action` |

质量维度：IDENTITY（身份一致性）、COMPOSITION（构图）、ACTION（动作）、CAMERA（摄影机）、PHYSICS（物理）、AUDIO_TIMING（音画同步）、CONTINUITY（连续性）。

**策略**：PASS/WARN 可正常采用；BLOCK 需要 `canvas:quality:override` 权限 + 非空原因 + 审计日志。质量报告不能自动创建导演台或自动采用。
...
|---|---|---|
| `id` | BIGINT PK | 自增主键 |
| `shot_unit_id` | BIGINT FK | 所属 ShotWorkUnit |
| `revision` | INT | 采用版本号（append-only，1, 2, 3...） |
| `candidate_id` | BIGINT FK | 被采用的候选 |
| `adopted_by` | BIGINT | 采用人 |
| `override_reason` | VARCHAR(500) | 强制采用原因（质检 BLOCK 时必填） |
| `created_at` | DATETIME | 采用时间 |

约束：每次采用创建新 revision；旧候选和历史采用记录不删除；节点 API 不能改变正式采用。

**DeliveryManifest** — 交付清单：

| 表 | 用途 |
|---|---|
| `delivery_manifests` | 交付清单：`project_id`、`revision`（append-only）、`manifest_hash`（SHA-256）、`status` |
| `delivery_manifest_items` | 清单条目：`shot_unit_id`、`adoption_id`、`asset_version_id`、`sort_order` |

**交付交换文件**：
- ZIP 素材包：版本化素材 + `manifest.json` + `checksums.txt`
- EDL（CMX3600）：按采用镜头顺序的时间线交换格式
- FCPXML 1.9：Final Cut Pro 交换格式，包含素材相对路径映射

> **[superpowers 更新 V1.8 — 冲突裁决]**：原口径为"导出项目素材包 ZIP"。最终决策为"交付清单（DeliveryManifest）+ ZIP 素材包 + EDL/FCPXML 交换文件"。平台不提供视频合成或成片导出。旧 compose/export API `[已废弃]`，保留审计读取。

#### 9.19.8 新增数据表汇总

V1.8 在 V1.5 基础上新增以下表（对应 Flyway 迁移 V12-V15）：

| 迁移 | 表 |
|---|---|
| V12 | `canvas_shot_units`、`generation_request_snapshots`、`generation_candidates`、`shot_adoptions`、`canvas_migration_reports` |
| V13 | `director_scenes`、`director_drafts`、`director_revisions`、`director_revision_assets` |
| V14 | `generation_task_attempts`、`model_capability_profiles` |
| V15 | `canvas_quality_reports`、`canvas_quality_issues`、`delivery_manifests`、`delivery_manifest_items` |

同时扩展：
- `canvas_projects`：增加 `mode`、`schema_version`、`migration_status`、`content_project_id`、`content_unit_id`、`content_version_id`
- `canvas_nodes`：增加 `shot_unit_id`、`node_schema_version`、`model_strategy_ref`
- `canvas_edges`：增加 `port_contract_version`、`migration_status`、`validation_result`
- `generation_tasks`：增加 `request_snapshot_id`、`actual_credit_cost`、`adapter_version`

#### 9.19.9 Feature Flag 与灰度

五开关，默认全部 `false`，按 Workspace 和 Canvas 灰度：

| Flag | 控制范围 |
|---|---|
| `CANVAS_KERNEL_V2` | 新生产内核主开关：ShotWorkUnit、双模式、候选、采用 |
| `TYPED_PORTS` | 类型化端口与方向校验 |
| `DIRECTOR_V2` | Three.js 导演台 + Draft/Revision 版本流 |
| `MODEL_ADAPTER_V2` | 能力编译、适配器预览与确认、不可变请求快照 |
| `QUALITY_DELIVERY_V2` | 质量报告、采用策略、交付清单、EDL/FCPXML |

灰度顺序：每个阶段先内部项目 → 小规模真实项目 → 扩大范围。

#### 9.19.10 绞杀迁移策略

四步流程：

1. **审计**：`CanvasLegacyAuditService` 扫描节点、连线、导演 JSON、任务和资产引用，生成分类报告
2. **影子读**：用新 ViewModel 读取旧数据，与旧页面摘要和节点计数对比，不修改数据
3. **单画布升级**：`POST /api/v1/canvas/projects/{projectId}/upgrade` — 事务内备份、写 ShotWorkUnit、端口契约、导演版本和候选映射
4. **灰度切换**：按 Workspace/Canvas 开启新内核；旧 API 转只读并记录调用

**原则**：新链路单写、旧链路只读兼容；升级前生成完整备份快照；歧义数据必须人工确认（`NEEDS_CONFIRMATION`），不静默转换或删除；回滚依靠备份和兼容投影。

#### 9.19.11 R0-R5 分阶段交付

| 阶段 | 目标 | 核心交付 |
|---|---|---|
| **R0：稳定与止损** | 恢复画布项目中心可信可用性 | 项目中心错误降级；统一 `director` 节点类型；隐藏假入口；Feature Flag 骨架 |
| **R1：可迁移生产内核** | 建立双模式、ShotWorkUnit、类型化端口、候选和采用 | V12 schema；探索/正式双模式；端口 registry；请求快照+候选+采用；单画布升级 |
| **R2：真实导演台** | Three.js 3D 工作区替代 DOM 原型 | V13 schema；Three.js 视口+Draft/Revision+乐观锁；时间线+预设+冻结检查 |
| **R3：模型与 Blender** | 真实可追溯候选，两条链路闭环 | V14 schema；Seedance Gate；CapabilityCompiler+ModelAdapter；Blender Worker+坐标转换；任务 Attempt+结算 |
| **R4：质量与交付** | 质检策略+外部后期交接 | V15 schema；质量报告+分级采用；DeliveryManifest；ZIP/EDL/FCPXML 打包；产品指标 |

阶段 Gate：
- **G0**：Seedance API 真实合同验证
- **G1**：资产签名 URL、版本哈希、真人肖像/声音授权
- **G2**：Blender Worker 镜像、资源配额、故障恢复压测
- **G3**：积分预估/冻结/实扣/退还/补偿闭环

详见各阶段实施计划：`docs/superpowers/plans/canvas/2026-07-05-canvas-production-kernel-r0-stabilization.md` ~ `r4-quality-delivery.md`。
...
> **详细设计见**：`docs/superpowers/specs/workbench/2026-07-04-production-sop-completion-design.md`
...
> **详细设计见**：`docs/superpowers/specs/workbench/2026-07-04-unified-task-event-center-design.md`
...
> **详细设计见**：`docs/superpowers/specs/workbench/2026-07-04-asset-generation-history-workbench-design.md`
...
> **详细设计见**：`docs/superpowers/specs/workbench/2026-07-04-enterprise-workbench-completion-design.md`
...
> **详细设计见**：`docs/superpowers/specs/canvas/2026-07-04-standalone-blank-canvas-design.md`
...
> **详细设计见**：`docs/superpowers/specs/workbench/2026-07-04-user-configurable-agent-center-design.md`
...
> **详见**：`docs/superpowers/specs/workbench/2026-07-04-unified-task-event-center-design.md`（对账引擎）、`docs/superpowers/specs/workbench/2026-07-04-enterprise-workbench-completion-design.md`（审批投影 Outbox 消费）
...
> **详见**：`docs/superpowers/specs/workbench/2026-06-28-unified-account-model-billing-design.md`、`docs/superpowers/specs/workbench/2026-07-04-enterprise-workbench-completion-design.md`
