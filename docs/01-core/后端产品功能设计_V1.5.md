...
|--------|---------|------|---------|:---:|
| **用户与账户模块** | `module.auth` + `module.user` | 注册/登录/认证/个人中心/企业中心/权限 | 用户、企业、角色权限 | P0 |
| **剧本生成模块** | `module.script` | 剧本资产生成编排、样章/故事圣经/章节/分镜Master、Prompt模板 | 剧本、分镜、AI编排 | P0 |
| **剧本仓库模块** | `module.script` | 剧本CRUD、4轴标签、版本管理、L0-L4资产 | 剧本、标签、版本、资产 | P0 |
| **交易与支付模块** | `module.trade` | 剧本交易市场、授权管理、支付、订单 | 交易、支付、授权 | P1 |
| **AI资产市场模块** | `module.generation` | 风格模型市场、角色/场景资产、提示词市场 | AI模型、资产、社区 | P1 |
| **画布视频工作台模块** | `module.canvas` | 画布编排、分镜管理、单镜头视频生成、采用版本、素材包导出 | 画布、分镜、视频、素材清单、导出 | P0 |
| **工业化生产SOP模块** | `module.sop` | 生产准入、审计返工、版本锁定、产能估算 | SOP、审计、权限 | P1 |
| **通知与消息模块** | `module.notify` | 站内通知、邮件、短信、浏览器推送 | 通知、消息 | P0 |
| **Agent与Skill模块** | `module.agent` | AI导演、Skill执行、任务编排、第三方Agent接入、工具调用 | Agent、Skill、OpenAPI、自动化 | P1 |
| **AI API 网关** | `new-api/` (Go, 独立部署) | 40+ AI 提供商聚合、渠道管理、配额计费 | AI 路由、计费、限流 | P0 |
...
`user-svc` 同时是 `8080` 用户端与 `3001` new-api 管理端的统一身份事实源。两个端口功能独立，但不得各自维护可登录的业务账号体系。
