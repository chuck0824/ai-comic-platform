...
| 扩展域：画布 | `canvas_projects` 增加内容项目归属字段；生产快照/源 diff/生产准入；浮动编辑器改造 | `platform-home-canvas-center-design.md`、`canvas-node-floating-editor-design.md` |
| 账户/计费模型 | 8080 不再维护用户/工作区/余额主数据，通过 BFF 适配层调用 3001 版本化 API | `unified-account-model-billing-design.md` |

> **注意**：本文档中标注 `[superpowers 更新]` 的段落为本次新增或修改内容。未标注的原有内容保持不变，作为历史基线参考。
